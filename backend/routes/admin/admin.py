...
@router.get("/admin/admins_student_view", response_class=HTMLResponse)
async def admins_student_view_page(request: Request, db: AsyncSession = Depends(get_db)):
	...
	admins_student_view_path = BASE_DIR / "templates" / "admins_student_view.html"
	response = FileResponse(admins_student_view_path)
	response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
	response.headers["Pragma"] = "no-cache"
	return response
