import { loadUsername, formatDate, formatDateTime, escapeHtml, showError } from './utils.js';
...
function renderListHeader(taskList) {
  ...
  let headerHTML = `
    ...
    <div class="mb-2">
      <span class="task-list-code">${escapeHtml(taskList.unique_link_code)}</span>
    </div>
    ...
    Promise.all([
      ...
      .catch(err => {
        ...
        if (err.message.includes('401') || err.status === 401) {
          ...
          showError(err.message || 'An unexpected error occurred.', 'list-header');
        }
      });
}