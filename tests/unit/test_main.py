...
class TestSubmitResult:
    async def test_no_session_returns_401(self, client, task, task_set):
        r = await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
                               json=_submit(task.id))
        assert r.status_code == 401

    async def test_success_with_iso_start_time(self, client, task, task_set, student_session):
        client.cookies.set("student_session", str(student_session.id))
        r = await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
    ...
    async def test_success_with_z_suffix_timestamp(self, client, task, task_set, student_session):
        client.cookies.set("student_session", str(student_session.id))
        r = await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
    ...
    async def test_invalid_start_time_falls_back_to_now(self, client, task, task_set, student_session):
        client.cookies.set("student_session", str(student_session.id))
        r = await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
    ...
    async def test_missing_start_time_uses_now(self, client, task, task_set, student_session):
        client.cookies.set("student_session", str(student_session.id))
        r = await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
                               json=_submit(task.id))
        client.cookies.clear()
    ...
    async def test_attempt_persisted_in_db(self, client, task, task_set, student_session, db_session):
        client.cookies.set("student_session", str(student_session.id))
        await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
    ...
    async def test_failure_attempt_persisted(self, client, task, task_set, student_session, db_session):
        client.cookies.set("student_session", str(student_session.id))
        await client.post(f"/api/sets/{task_set.unique_link_code}/tasks/{task.id}/submit-result",
